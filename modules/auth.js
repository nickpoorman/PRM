...
// app.get('/testv', function(req, res) {
...
app.post("/signup", function(req, res) {
  var params = ['firstName', 'lastName', 'email', 'password', 'username'];
  res.locals.params = {};
  ...
  req.assert('firstName', 'First name is too short.').notEmpty();
  req.assert('lastName', 'Last name is too short.').notEmpty();
  ...
  req.assert('username', 'Username is too short.').notEmpty();
  ...
  var newUser = new User({
    ...
    newUser.createConfirmationToken(function(newUser) {
      newUser.save(function(err) {
        if(err) {
          if(err.code === 11000) {
            /* This should prob get changed in the future to make 2 async queries.
                -one to check for the existance of the username
                -one to check for the existance of the email
              */
            var isEmail = err.err.match(/email/);
            var isUsername = err.err.match(/username/);
            var errors = [];
            if(isEmail) {
              errors.push({
                param: 'email',
        ...
        return res.render('auth/welcome', newUser.toJSON());
      });
    });
  });
});
...
/*
  ...
  - This usually gets called from a form doing a post on the website.
*/
// app.post("/verify-account", verifyAccount, function(req, res) {
//   return res.json(res.locals.bootstrapData.verifyAccount.status, res.locals.bootstrapData.verifyAccount);
// });
/*
  - This resource takes a token to enable the account after it is created.
  - This usually gets called from an email with a verify account url.
*/
app.get("/verify-account/:token", verifyAccount, function(req, res) {
